...
if uploaded_file1 and uploaded_file2:
    ...
    for idx, (uploaded_file, col) in enumerate(zip([uploaded_file1, uploaded_file2], [col_a, col_b]), start=1):
        with col:
            image, satellite = process_image_before_model(uploaded_file)
            st.subheader(f"📸 Uploaded Image {idx}")
            ...
            with st.spinner(f"🔧 Generating Roadmap {idx}..."):
                try:
                    ...
                    st.error(f"❌ Error in Image {idx}: {e}")
